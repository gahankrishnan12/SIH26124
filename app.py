...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Urban Intelligence Platform | SIH26124",
    page_icon="🚌",
    layout="wide",
...
with tab_traffic:
    ...
    st.markdown("---")
    st.caption(f"🛡️ **Data Integrity Filter**: Whitelisted vehicle classes `{VALID_VEHICLE_CLASSES}`. Pedestrian/person events ({traffic_summary['non_vehicle_events_excluded'] - traffic_summary['road_damage_events_excluded']}) and road damage ({traffic_summary['road_damage_events_excluded']}) are strictly separated from vehicular traffic metrics.")
...
with tab_damage:
    ...
    # Road damage KPI cards
    dk1, dk2, dk3, dk4 = st.columns(4)
    with dk1:
        st.metric("Total Road Damage Hazards", len(dam_events))
    with dk2:
        st.metric("High Severity Hazards", db_stats["by_severity"].get("high", 0), help="Critical potholes requiring immediate maintenance")
    with dk3:
        st.metric("Medium Severity Hazards", db_stats["by_severity"].get("medium", 0))
    with dk4:
        st.metric("Low Severity Hazards", db_stats["by_severity"].get("low", 0))
